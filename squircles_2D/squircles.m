% squircles_paris
    ...
    if ~exist('experiment','var')
        ...
        experiment.varying = 'mv';                                         % staircase value is shape mean
        experiment.nb_blocks = 30;                                         % number of blocks
    ...
    if ~exist('conditions','var')
        ...
        conditions = [];
        for i_block = randperm(nb_blocks)
    ...
    if ~exist('participant','var')
        ...
        participant.filename = {'data',sprintf('squirclespsych_%s',participant.name),1};
    end
    ...
    if ~exist('stimulus','var')
        ...
        stimulus.crange    = [0 1];                                  % c=0 blue    -> c=1 red
        stimulus.srange    = [1 2];                                  % s=1 square  -> s=2 circle
        stimulus.vrange    = [-.95 +.95];
    ...
    try
        ...
        switch(experiment.SR)
            case 0
                Screen(w,'DrawText','SQUARE/RED + CIRCLE/BLUE (left button) or SQUARE/BLUE + CIRCLE/RED (right button)',      300,350,            stimulus.whitecol);
            case 1
                Screen(w,'DrawText','SQUARE/BLUE + CIRCLE/RED (left button) or SQUARE/RED + CIRCLE/BLUE (right button)',      300,350,            stimulus.whitecol);
        end
        ...
        Screen(w,'DrawText','You will have to decide as fast as possible the shape and colour',     200,200,   stimulus.whitecol);
        ...
        for i_trial = (data.i_trial+1):size(conditions,1)
            ...
            vv          = conditions(i_trial,3);
            constraint  = conditions(i_trial,4);
            stimdur     = conditions(i_trial,5);
            mv          = variables.means(conditions(i_trial,6));
            ...
            Vcat = sign(rand-0.5);
            Ccats = sign(rand(1,nstims)-0.5);
            Scats = Vcat * Ccats;
            % actual values for color and shape
            % C has to be in [0:1] [blue:red]
            % S has to be in [1:2] [square:circle]
            % V is in [-1:1]
            %                      mean          var     nitems  constraint    critmean    critsd    range              nsamples
            V = mypseudorandrange( 2*Vcat*mv ,  vv,     nstims, constraint,   0.001,      0.001,    stimulus.vrange,   1);
            ...
            if isempty(respcode);
                ...
                % correct
                cor = (Vcat==1 && respcode==experiment.keyright) || (Vcat==-1 && respcode==experiment.keyleft);
            end
            % auditory feedback
            if experiment.audio
                if cor==1
            ...
            data.Mv(i_trial)           = mv;                               % mean
            data.Vv(i_trial)           = vv;                               % var
            data.tasknum(i_trial)      = strcmp(experiment.task,'shape');  % 0='color', 1='shape'
            data.setsize(i_trial)      = nstims;                           % nstims (constant)
            data.V(1:nstims,i_trial)   = V;                                % binded value
            data.C(1:nstims,i_trial)   = C;                                % color value
            data.S(1:nstims,i_trial)   = S;                                % shape value
            data.Vcat(i_trial)         = Vcat;                             % binded category
            data.Ccats{i_trial}        = Ccats;                            % -1=categ1 (blue),   1=categ2 (red)
            data.Scats{i_trial}        = Scats;                            % -1=categ1 (square), 1=categ2 (circle)
    ...
    for i_psych = 1:nb_psych
        for i_mean = 1:nb_means
            data.mcor(i_psych,i_mean) = mean(data.cor(data.psych==i_psych & data.Mv==variables.means(i_mean)));
        end
    end
