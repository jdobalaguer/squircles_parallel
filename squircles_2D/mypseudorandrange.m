function [zevalues] = mypseudorandrange(array_mean,array_sd,nitms,constraint,critmean,critsd,range,nsamples)
...
% range is the range, of course
    ...
    if array_sd==0
        ...
        while pass < nsamples
            counter = counter+1;
            if counter > 1000
                error(['mypseudorange: exit: 1000 loops(array_mean = ',num2str(array_mean),' ; range = ',num2str(range),')\n']);
            end
