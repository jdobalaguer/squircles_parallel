% squircles_paris
    ...
    try
        ...
        for i_trial = (data.i_trial+1):size(conditions,1)
            ...
            pic = Screen('GetImage',w,stimulus.rect);
            imwrite(pic, [num2str(npic),'.jpg']);
            npic = npic+1;
    ...
    for i_mcor = 1:size(data.mcor,1)
        ...
        %p = nlinfit(variables.means,data.mcor(i_mcor,:),shifted_sigmoid,[0,0,1,0]);
        fits_p = [fits_p ; p];
        %fits_y = [fits_y ; shifted_sigmoid(p,x)];
